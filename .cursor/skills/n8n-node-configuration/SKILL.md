...
**Harness:** Use with `workflows/<slug>.json` and specs. MCP tools are required in the pipeline when available; [`n8n-verify`](../n8n-verify/SKILL.md) is authoritative for spec alignment. `$node["..."]` must match story node names in ARCHITECTURE.
...
`patchNodeField` is strict — it errors if the find string isn't found or matches multiple times (unless `replaceAll: true`). This prevents accidental silent failures during configuration updates. See [n8n-mcp-local](../n8n-mcp-local/SKILL.md) and [WORKFLOW_GUIDE.md](../n8n-mcp-local/WORKFLOW_GUIDE.md) for full syntax and examples.
...
**Related Skills**:
- [n8n-mcp-local](../n8n-mcp-local/SKILL.md) — how to use discovery tools correctly
