...
## Expressions in node parameters (not Code node)

Use these in SharePoint, If, HTTP, and other node fields as `={{ … }}` in JSON — **not** inside Code `jsCode` (no `{{}}` there).

| Use case | Expression |
|----------|------------|
| File id from Get Many | `={{ $json['@odata.etag'].replace(/\"/g, '').split(',')[0] }}` |
| If filter folder in webUrl | `rightValue`: `02%20-%20Documentos%20Base` (encoded; display name `02 - Documentos Base` in UI only) |
| Upload name in loop | `={{ decodeURIComponent($('Loop over base documents').item.json.webUrl.split('/').pop()) }}` |

`$('Node Name')` must match `nodes[].name`. See [n8n-expression-syntax EXAMPLES ex16–ex19](../n8n-expression-syntax/EXAMPLES.md) and [docs/n8n-node-catalog.md](../../../docs/n8n-node-catalog.md).
