 ...
 *
 * Checks if the current n8n-harness-mcp version is outdated by comparing
...
export async function checkNpmVersion(forceRefresh: boolean = false): Promise<VersionCheckResult> {
  ...
  try {
    ...
    const response = await fetch('https://registry.npmjs.org/n8n-harness-mcp/latest', {
      headers: {
    ...
    const result: VersionCheckResult = {
      ...
      error: null,
      checkedAt: new Date(),
      updateCommand: isOutdated ? `npm install -g n8n-harness-mcp@${latestVersion}` : undefined
    };
