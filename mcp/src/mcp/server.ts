...
export class N8NDocumentationMCPServer {
  ...
  constructor(instanceContext?: InstanceContext, options?: MCPServerOptions) {
    ...
    logger.info('Initializing n8n-harness MCP server');
    
    this.server = new Server(
      {
        name: 'n8n-harness-mcp',
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
      ...
      const uiMatch = uri.match(/^ui:\/\/n8n-harness-mcp\/(.+)$/);
      if (uiMatch) {
      ...
      if (uri.startsWith('skill://n8n-harness-mcp/')) {
    ...
    switch (name) {
      ...
      case 'n8n_generate_workflow': {
        ...
        return {
          hosted_only: true,
          message:
            'The n8n_generate_workflow tool is not enabled in the n8n-harness vendored MCP build. ' +
            'Use harness workflow skills (n8n-plan, n8n-build) and template/search tools instead.',
        };
      }
...
`,
  ...
  async run(): Promise<void> {
    ...
    logger.info('n8n-harness MCP server running on stdio transport');
