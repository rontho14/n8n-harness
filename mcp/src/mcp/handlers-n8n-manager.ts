...
export async function handleHealthCheck(context?: InstanceContext): Promise<McpToolResponse> {
  ...
  try {
    ...
    if (versionCheck.isOutdated && versionCheck.latestVersion) {
      responseData.updateWarning = `⚠️  n8n-harness-mcp v${versionCheck.latestVersion} is available (you have v${versionCheck.currentVersion}). Update recommended.`;
    }
  ...
  if (mcpMode === 'http') {
    ...
    return {
      ...
      troubleshooting: [
        ...
        '2. Check MCP server entry: {"mcpServers": {"n8n": {"command": "npx", "args": ["-y", "n8n-harness-mcp"]}}}',
        '3. Restart Claude Desktop after config changes',
        '4. Check Claude Desktop logs for startup errors',
        '5. Test npx can run: npx -y n8n-harness-mcp --version',
...
export async function handleDiagnostic(request: any, context?: InstanceContext): Promise<McpToolResponse> {
  ...
  if (apiConfigured && apiStatus.connected) {
    ...
    diagnostic.troubleshooting = {
      ...
      documentation: 'mcp/THIRD_PARTY.md (upstream)'
    };
  } else {
