...
This document describes the STRIDE threat model for **n8n-harness-mcp**. It is intended for contributors and operators who want to understand the security assumptions the project makes, the trust boundaries it relies on, and the mitigations already in place.
...
n8n-harness-mcp is a Model Context Protocol server that gives AI assistants structured access to n8n node documentation and, optionally, management access to an n8n instance via its REST API. This threat model covers:

- The n8n-harness-mcp server itself, in all supported deployment modes (stdio, HTTP single-session, multi-tenant HTTP, Docker image).
- The data flows between AI clients, the server, n8n instances, and the n8n.io templates API.
- The supply chain used to publish the npm package and the `ghcr.io/czlonkowski/n8n-harness-mcp` container image.

It does **not** cover:

- The security of n8n itself. As stated in `SECURITY.md`, "the security boundary is n8n itself, not n8n-harness-mcp" — capabilities reachable through the n8n REST API are not re-evaluated here.
...
- **Docker image** — `ghcr.io/czlonkowski/n8n-harness-mcp`, runs as a non-root user with the randomized UID/GID created at build time.
...
flowchart LR
  ...
  subgraph ServerTrust["n8n-harness-mcp Trust Zone"]
...
| npm package `n8n-harness-mcp` | High | Downstream users execute it directly; integrity matters. |
| Docker image `ghcr.io/czlonkowski/n8n-harness-mcp` | High | Same — users pull and run it. |
...
- **Malicious workflow JSON served from `api.n8n.io` templates.** Mitigation: templates are stored as inert data and validated as JSON; they are never `eval`'d, `require`'d, or executed inside n8n-harness-mcp. Any execution happens later on the user's own n8n instance, which is out of scope per `SECURITY.md`.
...
- **Capability amplification on the downstream n8n (e.g., Code node execution).** Explicitly out of scope per `SECURITY.md` — n8n-harness-mcp grants no capability the caller does not already have on the n8n REST API.
