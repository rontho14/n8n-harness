...
n8n-harness-mcp is a proxy to the n8n REST API. **The security boundary is n8n itself, not n8n-harness-mcp.** Every operation available through n8n-harness-mcp can be performed identically using the n8n REST API with the same API key. n8n-harness-mcp does not grant any capability beyond what the n8n API already provides.
...
When n8n-harness-mcp returns data from an n8n instance (workflow names, descriptions, execution results), that content is surfaced to the LLM. If a malicious actor has access to modify workflows on the shared n8n instance, they could craft workflow names or descriptions designed to manipulate the AI assistant.
