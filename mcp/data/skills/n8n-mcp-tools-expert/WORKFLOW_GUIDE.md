...
n8n's public API has no native "which workflows use credential X" endpoint, so n8n-harness-mcp builds the reverse index for you by scanning workflows client-side. Pass `includeUsage: true` to either `list` or `get`.
